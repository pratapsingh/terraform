...
terraform {
  backend "s3" {
    bucket  = "be-iac-state"
    key     = "us-west-1/mgmt/network/terraform.tfstate"
    region  = "us-west-1"
    profile = "devops"
  }
}
...
data "terraform_remote_state" "prod-network" {
  backend = "s3"
  config {
    bucket  = "be-iac-state"
    key     = "us-west-1/prod/network/terraform.tfstate"
    region  = "us-west-1"
    profile = "devops"
  }
}
...
data "terraform_remote_state" "stage-network" {
  ...
  config {
    bucket  = "be-iac-state"
    key     = "us-west-1/stage/network/terraform.tfstate"
    region  = "us-west-1"
    profile = "devops"
  }
}
